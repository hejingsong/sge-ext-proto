...
int sge_encode(struct sge_proto *proto, const char *name, const void *ud,
               sge_fn_get fn_get, uint8_t *buffer) {}
int sge_decode(struct sge_proto *proto, uint8_t *bin, size_t len, void *ud,
               sge_fn_set fn_set) {}
...
void sge_print_protocol(struct sge_proto *p) {
  ...
  struct sge_radix_iter iter;

  sge_init_radix_iter(&iter, p->block_tree);
  while ((bp = (struct sge_block *)sge_next_radix_iter(&iter))) {
    printf("%s %d %d\n", bp->name, bp->id, bp->count);
    for (i = 0; i < bp->count; ++i) {
      printf("\t%s %d %d %d\n", bp->fields[i].name, bp->fields[i].id,
             bp->fields[i].tid, bp->fields[i].flags);
    }
  }
}
